...
pub async fn insert_blocks(running: Arc<AtomicBool>,
    ...
    while running.load(Ordering::Relaxed) {
        ...
        if insert_queue.len() >= max_queue_size || (insert_queue.len() >= 1 && Instant::now().duration_since(last_commit_time).as_secs() > 2) {
            ...
            con.transaction(|con| {
                ...
                if !vcp_started {
                    ...
                    if blocks_inserted > 0 && cb_cleared == 0 && ta_cleared == 0 { noop_delete_count += 1; } else { noop_delete_count = 0; }
                }
            ...
            if !vcp_started {
                info!("Committed {} new blocks, cleared {} cb and {} ta. Last block timestamp: {}", blocks_inserted, cb_cleared, ta_cleared,
            ...
            if vcp_started && Instant::now().duration_since(checkpoint_last_saved).as_secs() > CHECKPOINT_SAVE_INTERVAL {
                if checkpoint_hash.is_empty() {
                    ...
                    if checkpoint_hash_tx_committed_count == checkpoint_hash_tx_expected_count {
                        ...
                        if select(exists(chain_blocks::dsl::chain_blocks.filter(chain_blocks::block_hash.eq(&checkpoint_hash)))).get_result::<bool>(con).unwrap() {
