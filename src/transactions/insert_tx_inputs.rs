...
use crate::database::models::TransactionInput;
    ...
    loop {
        if insert_queue.len() >= INSERT_QUEUE_SIZE || (insert_queue.len() >= 1 && SystemTime::now().duration_since(last_commit_time).unwrap().as_secs() > 2) {
            ...
            con.transaction(|con| {
                ...
                transactions_inputs::dsl::transactions_inputs
                    .filter(transactions_inputs::transaction_id.eq_any(insert_queue.iter().map(|ti| ti.transaction_id.clone()).collect::<Vec<Vec<u8>>>())
                        .and(transactions_inputs::index.eq_any(insert_queue.iter().map(|ti| ti.index).collect::<Vec<i16>>())))
